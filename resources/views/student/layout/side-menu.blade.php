...
@section('head')
    <title>{{ $page_title.' :: '.config('app.student_title') }}</title>
@endsection

@section('content')
    @include('../student/layout/components/mobile-menu')
    <div class="flex">
        <!-- BEGIN: Side Menu -->
        <nav class="side-nav">
            <a href="" class="intro-x flex items-center pl-5 pt-4">
                <img alt="Midone Tailwind HTML student Template" class="w-6" src="{{ asset('dist/images/logo.svg') }}">
        ...
        <!-- BEGIN: Content -->
        <div class="content">
            @include('../student/layout/components/top-bar')
            @yield('subcontent')
        </div>
        <!-- END: Content -->
    </div>
